...
In section \@ref(regression) we briefly recap linear regression. As a real-world example we demonstrate the use of linear regression to predict gene expression values as a function of time. In this section we also demonstrate how, by breaking data into *training and test sets*, we can choose between models of increasing complexity in order to select one that is optimal in terms of predictive accuracy. Such *models* can be used to make predictions about the future (or at intermediate points lacking data) which may form the basis for automated decision making. Note that here we only intend to introduce the very basics, but decision making algorithm should be treated a some degree of skepticism with regard to their robustness and the ethical quandaries they pose.
...
From this we can see for ourslf that the data consists of several variables measured over a time course. In fact, this experiment consists of several time series, with measurements of Arabidopsis leaves in response to infection with a necrotophic fungus \emph{Botrytis cinerea}, and a second set of experiments containing gene expression in an uninfected (control) conditions. Each condition has 4 replicates, so $8$ time-series in total. The variables are represented columnwise, including time and gene experssion, all of which are continuous variables. Two variables, labeled as `Class' and `Infec' appear to be binary - we will get to these later. We can extract out the names of the variables (gene names) as a new variable in R, by taking the column names:

```{r echo=T}
genenames <- colnames(D)
```

We can also pull out the time variables of the control time series. From the structure of the data we know that the first 96 rows correspond to control, with the second 96 corresponding to infection.
...
```
This would tell us what colum contains the variable `Time'. We could then pull out the data:
```{r echo=T}
genenames <- colnames(D)
...
Before we get down to doing any real Machine Learning we first need to familiarise ourself with the data. In fact, it helps a lot if we come armed with a well thought out question: this will help us generate optimal datasets to begin with, and will guide what methods we use to analyse the dataset. As previously suggested, our question going into this is something like `how does gene expression change over time and in response to infection'.

Let's start by plotting one the gene expression variables (AT2G28890) as a function of time. The standard plotting we used throughout this course will be ggplot. It makes for very nice plotting, but can be somewhat obscure in syntax, so the code below is probably more opaque than is necessary.
```{r echo=T}
library(ggplot2)
ggplot(D, aes(x = Time, y = AT2G28890, colour = factor(Class)) ) + geom_point(size=2.5) + theme_bw()
```

So here Time is our explanatory variable, the variable that is generally easy to measure, and AT2G28890 represents our output variable, the one we're actually interested in. In the above plot we can see both a change in the variable over time, and a striking difference between the control versus infected time series. Depending on the number of variables we could do this for all variables and take a look at them one by one, but this is tedious when we have thousands or even tens of thousands of variables. A heatmap is a good way to visualise more genes simultaneously. In fact, let's take a look at the heatmap of the infected time series minus the control. For ease of interpretation we will do this for replicate one only:
...
In the above snipped we have additionally clustered the values to bring out the signal even more. We can see very clearly strong patterns in the data that show both up-regulation and down-regulation of genes over time. We can use ML to answer various questions about this data. In the next section we will undertake a very simple task: we will focus on the gene AT2G28890 and in either the control or infection time series we will try to identify the functional nature of the expression pattern.

### Linear regression {#linear-regression}

Now that we have an idea about what our dataset is, we can start to do something with it. Here we have a time series (a number of time-series, in fact), and want to develop an understanding of how specific genes are changing over time: this would allow us to predict what gene expression might be doing at some point in the future (forecasting) or uncover something about the physical nature of the system i.e., what kind of function best describes the behavior. To do so we first need a *model*. One of the simplest models we could assume is linear regression, which assumes that the variable of interest, denoted $y$, depends on an explanatory variable, $x$, via:
...
But how do we infer these parameters? The answer is we do so by empirically minimising/maximising some *objective function*. Specifically, for a given value of $m$ and $c$ we can make predictions about what value $y$ is at any value of $x$. We therefore split that data into two: a training set, $\{ \mathbf{X}_{train}, \mathbf{y}_{train}\}$, and a test set, $\{ \mathbf{X}_{test}, \mathbf{y}_{test}\}$. Using the training set we can can we can find a value of $m$ and $c$ such that the sum of the squared difference between predictions of the model at locations $\mathbf{X}_{train}$, denoted $\mathbf{y}^\prime$, and the actual observed values $\mathbf{y}_{train}$ are in some way minimal. This is the sum squared error. A number of other *objective functions* exist, each of which comes with their own set nuances. A key benefit of using the sum squared error is that it is mathematically tractable: that is we can directly solve the equation rather than having to do iterative searches.
...
```
Note that here we have manually selected the first three replicates from the infection time series (indexed by rows $97-168$) and thus have saved an entire replicate $4$ for evaluating performance. As an alternative, we could have instead randomly partitioned the data into a training set and test set: although there is no exact prescirption for doing this, and somewhere between a $60/40$ and $80/20$ split is common. If we went donwn this route, our code would look something like:

```{r echo=F, eval=F}
index <- sample(1:nrow(D),round(0.8*nrow(D)))
xtrain2 <- D[index,1]
ytrain2 <- D[index,geneindex]
xtest2 <- D[-index,1]
ytest2 <- D[-index,geneindex]
```

and voila, we have our training and test sets. Finally, an alternative way we could split the data is via the createDataPartition function:

```{r echo=F, eval=F}
index <- createDataPartition(y=D$Time, times=1, p=0.7, list=F)
xtrain2 <- D[index,1]
ytrain2 <- D[index,geneindex]
xtest2 <- D[-index,1]
ytest2 <- D[-index,geneindex]
...
An important side note is that here is that, on lines 96 we have set the random number generator - this will help ensure our code is repeatable. Another thing we will need to do to help make things repeatable is to know what package numbers we used. We can do so by printing the session info:
...
Conveniently, in cases where we do not specify otherwise, {caret} will perform $k$-fold cross validation on the training set, and we can look at various metrics on the held out data in {lrfit$results}. We can make predictions at new points (for example if we are interested in forecasting at some time in the future) by specifying a new set of time points over which to make a prediction:
...
```
In general the fit seems to capture a general downward trend. 
...
```
By and large, for a good model we would expect the residuals to look roughly random centred on $0$. If we see structure, this may be a clue that our model is not quite right. 

At this point we can calculate the mean squared error on the held out data:
```{r echo=T}
RMSE <- sqrt( mean( (forecastValues - D[169:192,geneindex])^2 ) )
```
At this stage we could maybe compare this to the values in the training set to get an idea of how good the model is. However, the error on held out data comes into its own when looking to compare models, as we shall see in the next section. 
...
As we increase the model complexity the fit may *appear* to match perfectly well in the training set. However, such models would be becomes completely useless for prediction. We are overfitting! This is why we use held out data, so that we can evaluate, empirically, when a model is useful, or when it is simply memorising the training set. 
...
```
Now things become a little more obvious. In this example polynomial of degree $4$ is the best fit.
We can plot the best models:
...
auc
```
Both the ROC curve and the AUC score are much better. In the example below, we now to regress infection status against individual gene expression levels for all genes in our set. The idea is to identify genes that have expression values indicative of *Botrytis* infection: marker genes.
...
```
This looks promising - there are a few combinations that allow us to make even better predictions. However, we have made one mistake. If we are to do this properly, we must ensure that we are making decisions on a dataset that has not been seen before by the model. In the above case, this would not be true,as we have selected our "best gene" based on the testing data. Thus to do the above properly we would have to either look at pairwise combinations earlier, or have saved a third batch of genes on which to do this selection. Indeed it is common to see things broken down into training, evaluation, and test sets. Luckily I have just such a dataset that I have been holding in reserve:
```{r echo=T}
...
```
Here we indicate by a vertical line the basal model (i.e, AT2G21380 only). We can see there are several combinatorial models that allow us to more acurately predict infection status. As with regression approaches, we could use other models to classify this dataset and use metrics on hold-out-data to empirically select the best model. 

## Using regression approaches to infer 3D gene expression patterns in marmoset embryos

In a recent paper Bergmann et al. (2022) combined laser capture microdissection (LCM) with RNA-sequencing anh immunofluorescent staining to generate 3D transcriptional reconstructions of early post-implantation marmoset embryos. Specifically, adjacent sections were used to build a 3D model of the embryo. The 3D position of each LCM within this reconstructed embryo was retained, allowing an interpolation of expression patterns across the embryo. Here we will take a look at the 3D models and use regression to investigate anterior-posterior gradients in the embryonic disc.

First we will load the 3D "scaffolds" for the embryonic disc and the amnion. This consists of a set of vetices and a set of indices that define a set of faces. These scaffolds were constructed from sequential sections of a Carnegie stage 6 marmoset embryo. 
...
We will also load in the 3D locations of the laser capture microdissection samples that we used to do RNA-seq and the gene-expression of those samples. For simplicity I've included the processed expression levels of a handful of genes. 

```{r echo=T}
D5 <- read.table("data/Embryo/CS6Expression.csv",sep=",",header=T)
```

As an illustrative example of regression we will look at the expression pattern differences in  the embryonic disc vs amnion. We first need to pull out a training set, in this case we will do regression on embryonic disc and amnion seperately. 
...
```{r echo=T}
ax <- list(title = "",showgrid = FALSE,zeroline = FALSE)

plot_ly(x = D1$V1, y = D1$V2, z = D1$V3,  
        i = c(D2$V1-1,D3$V1-1),j=c(D2$V2-1,D3$V2-1),k=c(D2$V3-1,D3$V3-1),
        facecolor = c(rep("#0767DA",dim(D2)[1]), rep("#5F54C7",dim(D3)[1]) ),
        type = "mesh3d") %>% layout(scene = list(xaxis = ax, yaxis = ax, zaxis = ax), font = list(color='#FFFFFF'))
```

We can visualise where the LCM samples are within the embryo:
...
p <- plot_ly(x = D1$V1, y = D1$V2, z = D1$V3,  
             ...
             opacity =0.9,
             type = "mesh3d") %>% layout(scene = list(xaxis = ax, yaxis = ax, zaxis = ax), font = list(color='#FFFFFF'))
p <- p %>% add_markers(data=D5, x=D5$X[ind1],y=D5$Y[ind1],z=D5$Z[ind1],type="scatter3d", type   = 'scatter', mode   = 'markers', opacity =0.9, marker = list(size = 5))
```



Okay so now we can do some regression. In our paper we used a more complicated form of regresion called Gaussian process regression, a Bayesian to inference of nonlinear functions. Here, however, we will use linear regression to identify gradients within the embryonic disc. We first infer a model using the embryonic disc samples, and then infer a value over the full scaffold regiono 
...
Below we instead look at the expression level of T. 
...
Finally, in the snippet of code we visualise expression of SOX2 on both the embryonic disc and amnion.
...
Whilst in these examples we have not used a rigorous treatment of the data, they do illustrate the power of regression. Indeed in our paper we make use of different regression models to statistically identify tissue that exhibit strong spatial variation.
 
In the next section we will move on to a very flexible and widely used class of models that can be used for both regression and classification: neural networks.
